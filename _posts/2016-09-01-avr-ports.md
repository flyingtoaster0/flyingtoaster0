...
So it's looking like there's some actual bit shifting going on here. Based on the definition in `keypad.h`, the condition inside the if statement could be written `!((1<<4)&PINB)`. Clearly the `_BV(PB0)` in my code was somehow related to this bit shifting, so I changed my code up a bit:
...
`avr/iom32u4.h` is the file that includes definitions for the ATmega32u4. In it, we find the following definitions:
